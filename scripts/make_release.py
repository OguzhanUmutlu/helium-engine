...
def zip_folder(d):
    with ZipFile(d[:-1] + ".zip", "w", ZIP_DEFLATED) as zip:
...
os.remove(f"{dlat_dev}/win32.zip")
os.remove(f"{dlat_dev}/linux.zip")
shutil.copy(f"{script_dir}/../.stuff/SDL2.dll", f"{dlat_dev}/win32/SDL2.dll")
shutil.copy(
    f"{script_dir}/../.stuff/SDL2_image.dll", f"{dlat_dev}/win32/SDL2_image.dll"
)
